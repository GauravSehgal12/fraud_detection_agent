...
from src.services.risk_fusion import RiskFusion
...
class FraudInvestigationTools:
    def __init__(
        ...
        if (
            ...
            device_data = self.identity[["TransactionID", "DeviceInfo"]].drop_duplicates(
        ...
        elif "DeviceInfo" not in history.columns:
        ...
        if model is not None:
            self.risk_engine = RiskEngine(model=model, feature_builder=self.feature_builder)
        ...
        self.risk_fusion = risk_fusion if risk_fusion is not None else RiskFusion()
        self.decision_engine = (
            decision_engine
            if decision_engine is not None
            else DecisionEngine(
                review_threshold=self.risk_fusion.review_threshold,
                high_threshold=self.risk_fusion.high_threshold,
            )
        )
    ...
    def _prepare_new_transaction(self, transaction: dict[str, Any]) -> dict[str, Any]:
        ...
        if base_transaction is not None:
            ...
            enriched["TransactionID"] = transaction.get("TransactionID", enriched.get("TransactionID"))
    ...
    def get_card_history(self, transaction: int | dict[str, Any]) -> dict[str, Any]:
        ...
        if history.empty:
            return {"card1": int(card1), "transaction_count": 0, "average_amount": 0.0, "max_amount": 0.0, "min_amount": 0.0}
    ...
    def get_device_history(self, transaction: int | dict[str, Any]) -> dict[str, Any]:
        ...
        if self.identity is None or "TransactionID" not in self.identity.columns or "DeviceInfo" not in self.identity.columns:
        ...
        history = device_history[(device_history["DeviceInfo"] == device_info) & (device_history["TransactionDT"] < transaction_dt)]
        if history.empty:
            return {"device_info": device_info, "transaction_count": 0, "unique_cards": 0}
        return {"device_info": device_info, "transaction_count": int(len(history)), "unique_cards": int(history["card1"].dropna().nunique())}

    def get_cold_start_status(self, transaction: int | dict[str, Any]) -> dict[str, Any]:
        tx_data = self.get_transaction(transaction)
        if "error" in tx_data:
            return {"is_new_card": True, "is_new_device": True, "is_new_card_device_pair": True, "card_history_available": False, "device_history_available": False, "card_device_history_available": False}
    ...
    def get_risk_fusion(self, transaction: int | dict[str, Any]) -> dict[str, Any]:
        risk_res = self.get_risk_assessment(transaction)
        if "error" in risk_res:
            return risk_res
        behavioral = self.get_behavioral_risk(transaction)
        return self.risk_fusion.combine(
            model_score=float(risk_res.get("risk_score", 0.0)),
            behavioral_score=float(behavioral.get("behavioral_risk_score", 0.0)),
        )

    def get_rules_triggered(self, transaction: int | dict[str, Any]) -> list[dict[str, Any]]:
        return self.get_behavioral_risk(transaction).get("rules_triggered", [])

    def get_final_decision(self, transaction: int | dict[str, Any]) -> str:
        fusion = self.get_risk_fusion(transaction)
        if "error" in fusion:
            return "REVIEW"
        return self.decision_engine.decide(
            model_risk_score=fusion["raw_model_score"],
